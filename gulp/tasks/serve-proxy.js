...
module.exports = function proxyMiddleware(options) {
    ...
    return function (req, resp, next) {
        ...
        var myReq = request(opts, function (myRes) {
            ...
            if (!config.api.endpointsError && myRes.statusCode != 200) {
                next();
                return;
            }

            var fileName = 'api' + url.replace('?', '.') + '.' + req.method.toLowerCase() + '.json';
            if (!fs.existsSync(fileName)) {
                myRes.on('end', next);
                mkpath(fileName.substr(0, fileName.lastIndexOf('/')), function (err) {
                    if (err) throw err;
                    myRes.pipe(fs.createWriteStream(fileName));
                });
